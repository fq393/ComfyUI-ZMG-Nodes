...
- **Qwen Image 2.1 Edit Prompt Enhancer (GPUStack Vision)** - 使用同一个 `qwen3.8-27b` 看参考图后改写图生图指令
  - 接入 `IMAGE`（1–10 张的图像批次）和原始编辑要求；输出 `enhanced_prompt`、`wh_ratio`、`ratio_follow`，其中提示词可接图生图节点
  - 为降低视觉请求体大小，仅发送最长边 1024 像素的 JPEG 副本给 27B；原始高分辨率图片仍应另行连接到图生图节点
  - 采用 Qwen 官方 PE-I2I 的“明确修改项、保留未修改项、独立画幅字段”思路，但 **27B 通用模型不等同于官方微调的 PE-I2I 权重**；效果须通过固定原图、参数与种子的 A/B 对比确认
  - 默认中等思考；可在节点中调整系统提示词和思考档位。密钥同样优先建议由服务端 `ZMG_GPUSTACK_API_KEY` 提供，避免随工作流导出
